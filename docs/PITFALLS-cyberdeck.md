# Pitfalls — ghOSt boot fix (ported from rg35xxh-cyberdeck, verified May 2026)

> ghOSt `BOOT_FLOW=rocknix` (default) applies every fix below. `BOOT_FLOW=legacy`
> (KNULLI 4.9 + GPT) is kept only for experiments and does not boot reliably.
